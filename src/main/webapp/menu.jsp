...
<ul style="padding-left:0px;" class="list-group">
	<li class="list-group-item tree-closed" >
			<a href="${ItemPath }${root.url}"><i class="glyphicon glyphicon-dashboard"></i>${root.name }</a> 
		</li>
	<c:forEach items="${root.children }" var="permission">
	<c:if test="${empty root.children }">
		<li class="list-group-item tree-closed" >
			<a href="${ItemPath }${permission.url}"><i class="glyphicon glyphicon-dashboard"></i>${permission.name }</a> 
		</li>
	</c:if>
		...
		<ul style="margin-top:10px;display:none;">
			<c:forEach items="${permission.children }" var="child">
				<li style="height:30px;">
					<a href="${ItemPath }${child.url}"><i class="glyphicon glyphicon-user"></i> ${child.name }</a> 
				</li>
			</c:forEach>
		</ul>
	</li>
