            ...
            $(function () {
			    ...
			    $("#btnUpdate").click(function(){
			    	var nameVal = $("#name").val();
			         if(nameVal==""){
			        	 layer.msg("许可名称不能为空!", {time:1000, icon:0, shift:5}, function(){});
			             return;
			         }
			         var urlVal = $("#url").val();
			         if(urlVal==""){
			        	 layer.msg("许可路径不能为空!", {time:1000, icon:0, shift:5}, function(){});
			             return;
			         }
			    	$.ajax({
			    		url:"${ItemPath}/permission/updatePermission",
			    		type:"post",
			    		data:$("#updateForm").serialize(),
			    		success:function(result){
			    			if(result.flag){
			    				layer.msg("修改许可成功!", {time:1000, icon:6}, function(){});
			    				window.location.href="${ItemPath}/permission/index";
			    			}else{
