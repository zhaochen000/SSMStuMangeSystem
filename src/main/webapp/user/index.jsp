         ...
         var queryFlag = false;
            ...
            function delUsers(){
            	layer.confirm("是否删除选中的用户?",  {icon: 3, title:'提示'}, function(cindex){
    			    layer.close(cindex);
    			    $.ajax({
    			    	url:"${ItemPath}/user/deleteUsers",
    			    	type:"post",
    			    	data:$("#delForm").serialize(),
    			    	success:function(result){
    			    		if(result.flag){
    			    			layer.msg("删除成功!", {time:1000, icon:0, shift:6}, function(){});
    			    			window.location.href="${ItemPath}/user/index";
    			    		}else{
            ...
            function updateUser(uid){
            	window.location.href="${ItemPath}/user/update?uid="+uid;
            }
            function assignRole(uid){
            	window.location.href="${ItemPath}/user/assignRole?uid="+uid;
            }
            <!--删除-->
            			...
            			if(result.flag){
            				...
            				$.each(result.obj.list,function(index,user){
		                        ...
		                        tableStr+="<td>"+(index+1) +"</td>";
		          				tableStr+="<td><input type='checkbox' class='delCheck' name='uids' value='"+user.uid+"'></td>";                  
