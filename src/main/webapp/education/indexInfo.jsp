...
<button type="button" class="btn btn-danger" style="float:right;margin-left:10px;" onclick="delNewInfos()"><i class=" glyphicon glyphicon-remove"></i> 删除</button>
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${ItemPath}/education/newInfoAdd'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
    ...
    <script type="text/javascript">
         var queryFlag = false;
           ...
           function delNewInfos(){
           	layer.confirm("是否删除选中的公告信息?",  {icon: 3, title:'提示'}, function(cindex){
   			    layer.close(cindex);
   			    alert($("#delForm").serialize());
   			    $.ajax({
   			    	url:"${ItemPath}/education/delNewInfos",
   			    	type:"post",
           ...
           function updateNewInfo(infoID){
           	window.location.href="${ItemPath}/education/update?infoID="+infoID;
           }
          
           <!--删除-->
           function deleteNewInfo(infoID){
           	layer.confirm("是否删除该用户?",  {icon: 3, title:'提示'}, function(cindex){
   			    layer.close(cindex);
   			    $.ajax({
   			    	url:"${ItemPath}/education/delete",
   			    	type:"post",
   			    	data:{"infoID":infoID},
   			    	success:function(result){
   			    		if(result.flag){
           ...
           function queryByPage(nowPage){
            ...
            var jsonData = {"nowPage":nowPage};
           	...
           	$.ajax({
           		url:"${ItemPath}/education/indexByPage",
           		type:"post",
           		...
           		success:function(result){
           			if(result.flag){
           				var tableStr = "";
           				$.each(result.obj.list,function(index,newinfo){
           					//生成jQuery对象，进行装配或者使用html方法拼字符串
	            				tableStr+="<tr>";
		                        tableStr+="<td>"+(index+1) +"</td>";
		          				tableStr+="<td><input type='checkbox' class='delCheck' name='newInfos' value='"+newinfo.infoID+"'></td>";                  
		                        tableStr+="<td>"+newinfo.infoName+"</td>";
		                        tableStr+="<td>"+newinfo.infoAdd +"</td>";
		                        tableStr+="<td>"+newinfo.publishDate +"</td>";
		                        tableStr+="<td>";	
		          				tableStr+="<button type='button' onclick='updateNewInfo("+newinfo.infoID+")' class='btn btn-primary btn-xs'><i class='glyphicon glyphicon-pencil'></i></button>";
		          				tableStr+="<button type='button' onclick='deleteNewInfo("+newinfo.infoID+")' class='btn btn-danger btn-xs'><i class='glyphicon glyphicon-remove'></i></button>";
		          				tableStr+="</td>";
	                          	tableStr+="</tr>";
